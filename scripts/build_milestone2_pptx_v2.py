...
V2 visual polish only — technical content identical to V1.
Saves to (does NOT overwrite V1):
  C:\\Users\\audre\\OneDrive\\1- Final project\\Milestone2_HealthcareAI_V2.pptx
"""
...
OUT = Path(r"C:\Users\audre\OneDrive\1- Final project\Milestone2_HealthcareAI_V2.pptx")

# ---- Palette (same blue identity, clearer yellow/green highlights) ----
NAVY = RGBColor(0x0F, 0x4C, 0x81)
DARK = RGBColor(0x11, 0x18, 0x27)
MUTED = RGBColor(0x47, 0x55, 0x69)
WHITE = RGBColor(0xFF, 0xFF, 0xFF)
YELLOW = RGBColor(0xFD, 0xE0, 0x47)  # implemented — more vivid for instructor template
GREEN = RGBColor(0x86, 0xEF, 0xAC)  # ideal — more vivid
YELLOW_EDGE = RGBColor(0xCA, 0x8A, 0x04)
GREEN_EDGE = RGBColor(0x16, 0xA3, 0x4A)
...
def add_footer(slide, prs, text="Milestone 2 · Audrey Rah · University of Houston · AI Engineering · Healthcare AI CDSS"):
    rule = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.45), prs.slide_height - Inches(0.48), Inches(12.4), Inches(0.015))
    fill_shape(rule, BORDER)
    rule.line.fill.background()
    box = slide.shapes.add_textbox(Inches(0.45), prs.slide_height - Inches(0.40), Inches(12.4), Inches(0.28))
    p = box.text_frame.paragraphs[0]
    set_run(p.add_run(), text, size=9, color=MUTED)


def title_bar(slide, prs, title: str, subtitle: str = ""):
    # Soft slide canvas for consistent margins / readability
    canvas = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, prs.slide_width, prs.slide_height)
    fill_shape(canvas, LIGHT_BG)
    ...
    accent = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, Inches(0.92), prs.slide_width, Inches(0.07))
    fill_shape(accent, CYAN)
    accent.line.fill.background()
    box = slide.shapes.add_textbox(Inches(0.5), Inches(0.22), Inches(12.3), Inches(0.55))
    set_run(box.text_frame.paragraphs[0].add_run(), title, size=22, bold=True, color=WHITE)
    if subtitle:
        sub = slide.shapes.add_textbox(Inches(0.5), Inches(1.12), Inches(12.3), Inches(0.32))
...
def build():
    ...
    # Instructor legend (required clarity)
    legend_bg = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.4), Inches(1.18), Inches(12.5), Inches(0.48))
    fill_shape(legend_bg, WHITE)
    legend_bg.line.color.rgb = BORDER
    for i, (swatch, edge, label) in enumerate(
        [
            (YELLOW, YELLOW_EDGE, "Yellow = Implemented in Milestone 2"),
            (GREEN, GREEN_EDGE, "Green = Ideal Future Architecture"),
        ]
    ):
        left = Inches(0.6 + i * 6.1)
        chip = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, Inches(1.28), Inches(0.55), Inches(0.28))
        fill_shape(chip, swatch)
        chip.line.color.rgb = edge
        chip.line.width = Pt(1.5)
        tb = s.shapes.add_textbox(left + Inches(0.65), Inches(1.28), Inches(5.2), Inches(0.3))
        set_run(tb.text_frame.paragraphs[0].add_run(), label, size=12, bold=True, color=DARK)

    rows, cols = 10, 7
    table_shape = s.shapes.add_table(rows, cols, Inches(0.35), Inches(1.78), Inches(12.6), Inches(4.4))
    ...
    for j, h in enumerate(headers):
        ...
        set_cell_text(cell, h, size=8, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    for i, row in enumerate(data):
        for j, (text, kind) in enumerate(row):
            ...
            if kind == "y":
                ...
                set_cell_fill(cell, WHITE if j == 0 else LIGHT_BG)
            set_cell_text(
                cell,
                text,
                size=7 if j > 0 else 8,
                bold=(j == 0 or kind in ("y", "g")),
                color=DARK,
                align=PP_ALIGN.LEFT if j == 0 else PP_ALIGN.CENTER,
            )

    qa = s.shapes.add_textbox(Inches(0.4), Inches(6.3), Inches(12.5), Inches(0.7))
    tf = qa.text_frame
    tf.word_wrap = True
    set_run(
        tf.paragraphs[0].add_run(),
    ...
    prs.core_properties.title = "Milestone 2 — Healthcare AI CDSS (V2)"
    prs.core_properties.author = "Audrey Rah"
    prs.core_properties.comments = "Visual polish only; technical content unchanged from V1."
    prs.save(str(OUT))
    print(f"Saved: {OUT}")
